...
class WorkExperience extends Component {
    ...
    renderWorkExperienceItem(work_experience_item, index) {
        ...
        if (work_experience_item.start_date) {
            const starting_date = new Date(work_experience_item.start_date);
            starting_date_string = starting_date.toLocaleDateString(
                this.language,
                options
            );
        } else {
        ...
        if (work_experience_item.finish_date) {
            const finishing_date = new Date(work_experience_item.finish_date);
            finishing_date_string = finishing_date.toLocaleDateString(
                this.language,
                options
            );
        } else {
            finishing_date_string = this.translateString('current');
        }
            ...
            <div key={index} className="line row d-flex">
                ...
                <div className="col-md-8 content-wrap bg1">
                    <div className="line-content line-content-education">
                        {/* Work Place */}
                        <h3 className="section-item-title-1">{company}</h3>
                        {/* work place */}
                        {/* Graduation time */}
                        <h4 className="job">
...
function mapStateToProps(state) {
    ...
    const professional_experience =
        data && data.work_experience ? data.work_experience : null;
