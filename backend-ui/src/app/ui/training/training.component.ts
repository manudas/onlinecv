...
type StoreType = { locale: LocaleStore } & {
    trainings: { official: TrainingInterface[] } & {
        computer: TrainingInterface[];
    } & { other: TrainingInterface[] };
};
@Component({
...
export class TrainingComponent implements OnInit {
    ...
    constructor(
        ...
        this.officialData$ = this.store.pipe(
            select((state) => state?.trainings?.official)
        );
        this.computerData$ = this.store.pipe(
            select((state) => state?.trainings?.computer)
        );
        this.otherData$ = this.store.pipe(
            select((state) => state?.trainings?.other)
        );
    ...
    ngOnInit(): void {
        ...
        this.officialData$.subscribe(
            (data: TrainingInterface[]) =>
                data ? (this.officialData = data) : null
        );
        this.computerData$.subscribe(
            (data: TrainingInterface[]) =>
                data ? (this.computerData = data) : null
        );
        this.otherData$.subscribe(
            (data: TrainingInterface[]) =>
                data ? (this.otherData = data) : null
        );
    ...
    openTrainingDialog(
        data: EditTrainingStructure | string
    ): void {
    ...
    isTrainingEdit(
        data:
            ...
            | EditTrainingStructure
            | Object = {}
    ): data is EditTrainingStructure {
        return (
            (data as EditTrainingStructure).index !==
            undefined
        );
    }
    ...
    editTrainingValues(
        index: number,
        trainingData: TrainingInterface
    ) {
    ...
    openTrainingRemovalConfirmDialog(
    ...
    onDragStart($event) {
        const draggingElement: HTMLElement =
