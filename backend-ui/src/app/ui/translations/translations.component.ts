...
export class TranslationComponent implements OnInit {
    ...
    constructor(
        ...
        this.missingData$ = this.store.pipe(
            select(
                (state) =>
        ...
        this.translatedData$ = this.store.pipe(
            select(
                (state) =>
                    state?.translation?.translationManager
                        ?.translated
    ...
    ngOnInit(): void {
        this.selectedLocale$.subscribe(
            (data: string) => (this.selectedLocale = data)
        );

        this.missingData$.subscribe(
            (data: TranslationInterface[]) =>
                data ? (this.missingData = data) : null
        );
        this.translatedData$.subscribe(
            (data: TranslationInterface[]) =>
                data ? (this.translatedData = data) : null
        );
    ...
    fetchData() {
        ...
        switch (this.type) {
            case TranslationEnum.all:
                actions.push(
                    TRANSLATION_ACTIONS.FETCH_TRANSLATED_TRANSLATIONS(
                        {
            ...
            case TranslationEnum.missing:
                actions.push(
                    TRANSLATION_ACTIONS.FETCH_MISSING_TRANSLATIONS(
                        {
                            iso: this.selectedLocale
                        }
                    )
        ...
        actions.forEach((single_action) =>
            this.store.dispatch(single_action)
        );
    }
    ...
    isType(typeString: string) {
        return (
            this.type === TranslationEnum.all ||
            this.type === TranslationEnum[typeString]
        );
    }

    TranslationTypes: Number[] = Object.keys(
        TranslationEnum
    )
        .filter((e: any) => !isNaN(e))
        .map(Number);
    TranslationTypeStrings = Object.keys(
        TranslationEnum
    ).filter((e: any) => isNaN(e));

    getSource(type: number) {
        return this[
            `${this.TranslationTypeStrings[type]}Data`
        ];
    }

    getColumns(type: number) {
        return this[
            `${this.TranslationTypeStrings[type]}Cols`
        ];
    }

    dispatchSave(data: EditTranslationStructure) {
        this.store.dispatch(
            TRANSLATION_ACTIONS.SAVE_TRANSLATION({
                translation: data
            })
        );
    }
    ...
    deleteTranslation(id: string) {
        this.store.dispatch(
            TRANSLATION_ACTIONS.DELETE_TRANSLATION({
                translation: id
            })
        );
    }
    ...
    hasActions(type) {
        return (
            TranslationEnum[type] ===
            TranslationEnum[TranslationEnum.translated]
        ); // only translated has actions
    }
