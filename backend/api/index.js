...
var engine = require('./api_engine');

// middleware that is specific to this router
router.use(function timeLog(req, res, next) {
    let now = dateTime.create();
    let now_formatted = now.format('d/m/Y H:M:S');
    console.log('Requested API URL:', req.originalUrl, 'Time:', now_formatted);
    next();
});
...
router.get('/:command/:data', function (req, res) {
    console.log('Received command:', req.params.command, 'Data:', req.params.data, 'Query parameters:', req.query);
    const response_data_promise = engine.get(req.params.command, req.params.data, req.query);
...
router.get('/:command', function (req, res) {
	console.log('Received command:', req.params.command, 'Query parameters:', req.query);
	const response_data_promise = engine.get(req.params.command, null, req.query);
	if (!response_data_promise){ // not appropriate command
        res.status(404).send('<h1>404/Not found</h1>');
        return;
    }
