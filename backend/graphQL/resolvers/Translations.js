...
module.exports = {
    Query: {
        translations: async (
            ...
            if (modules.length !== tags.length) {
                throw new Error(
                    'Both modules and tags arrays must be size coincident and map each index to a domain/module/tag triplet'
                );
            }

            const translationFilter = modules.map(
                (module, index) => ({
                    module,
                    tag: tags[index],
                    missing: null,
            ...
            const missingTranslationFilter = modules.map(
                (module, index) => ({
                    module,
                    tag: tags[index],
                    missing: true,
            ...
            const translationList = await findAndUpdateMany(
                TranslationsModel,
                {
                    $and: [
                        {
            ...
            const resultMissingTrue =
                await findAndUpdateMany(
                    TranslationsModel,
                    {
                        $and: [
                            {
            ...
            const elementFoundInTranslationOrMissingTrue = (
                element
            ) =>
                translationList.some(
                    (translatedElement) => {
                        // is element in the translated list?
                        return (
                            translatedElement.domain ===
                                element.domain &&
                            translatedElement.module ===
                                element.module &&
                            translatedElement.tag ===
                                element.tag
                        );
                    }
                ) ||
                resultMissingTrue.some(
                    (missingTruetranslatedElement) => {
                        // is element in the missing: true translated list?
                        return (
                            missingTruetranslatedElement.domain ===
                                element.domain &&
                            missingTruetranslatedElement.module ===
                                element.module &&
                            missingTruetranslatedElement.tag ===
                                element.tag
                        );
                    }
                );
            ...
            const missingTranslations =
                translationFilter.length
                    ? translationFilter.reduce(
                          (
                              untranslatedList,
                              currentTranslation
                          ) => {
                              const elementIndex =
                                  elementFoundInTranslationOrMissingTrue(
                                      currentTranslation
                                  )
                                      ? untranslatedList.findIndex(
                                            (elem) =>
                                                elem.domain ===
                                                    currentTranslation.domain &&
                                                elem.module ===
                                                    currentTranslation.module &&
                                                elem.tag ===
                                                    currentTranslation.tag
                                        )
                                      : -1;
                              if (elementIndex !== -1) {
                                  untranslatedList.splice(
                                      elementIndex,
                                      1
                                  );
                              }
            ...
            const missingDocuments =
                missingTranslations.map((element) => ({
                    ...element,
                    missing: true,
                    language: lang,
                    accessCounter: 1,
    ...
    Mutation: {
        putTranslation: async (
            ...
            const element =
                await TranslationsModel.findOneAndUpdate(
                    {
        ...
        removeTranslation: async (
            ...
            throw new Error(
                `Translations with id: ${id} does not exist`
            );
        }
    }
};
